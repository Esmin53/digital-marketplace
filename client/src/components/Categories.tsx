  ...
  return (
        ...
        <div className="w-full max-w-7xl py-2 lg:py-4 xl:py-6 flex flex-col gap-4 justify-center items-center">
            ...
            <div className="w-full grid gap-4 grid-cols-2 sm:grid-cols-5">
                {CATEGORIES.map((item) => <div key={item.id} className="flex-1 aspect-square sm:max-w-32 md:max-w-36 lg:max-w-48 xl:max-w-52 bg-secondary rounded-lg flex flex-col px-1 justify-center items-center font-smooch shadow-sm gap-2 lg:gap-4 cursor-pointer hover:-translate-y-4 duration-300 border border-border">
                    ...
                    <Link to={`/products?category=${item.slug}`} className="flex items-center justify-center gap-1 text-accent-gray font-rubik text-sm lg:text-base">
                        View More
                        <HiMiniArrowRight />
                    </Link>
                </div>)}
